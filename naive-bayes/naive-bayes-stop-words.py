...
# get list of stop words from stop words file, and dont use those in training
stop_words = list()
with open("../HamSpam/stop-words.txt", 'r') as sw:
    stop_words = sw.readlines()
# removing all new lines from the words
...
for ham_filename in ham_email_files:
    with open(os.path.join("../HamSpam/ham", ham_filename), 'r') as h:
        ...
        for ham_line in ham_file_lines:
            ...
            if ham_line.lower() in stop_words:
                continue
            elif ham_line in stop_words:
                continue
            elif ham_line in ham_dict:
...
for spam_filename in spam_email_files:
    with open(os.path.join("../HamSpam/spam", spam_filename), 'r') as s:
        ...
        for spam_line in spam_file_lines:
            ...
            if spam_line.lower() in stop_words:
                continue
            elif spam_line in stop_words:
                continue
            elif spam_line in spam_dict:
...
for test_email in test_files:
    ...
    with open(os.path.join("../HamSpam/test", test_email), 'r') as te:
        ...
        for line in lines:
            line = line.strip()
            # if we see a stop word, we want to skip it (ignore when calc. probabilities)
...
# P = condition negative is a SPAM email (R) = # of file names in the truthfile
# R = condition positive is a HAM email = # of test files (100) - # of files in the truthfile (R)
